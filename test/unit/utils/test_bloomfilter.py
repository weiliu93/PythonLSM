...
from bloomfilter_hash_functions import Murmur32HashFunction, MD5HashFunction, SHA1HashFunction


def test_basic_add_and_existing_check():
    func_dict = {
        Murmur32HashFunction: 3,
        MD5HashFunction: 3,
        SHA1HashFunction: 3
    }
...
def test_bloom_filter_or():
    ...
    assert id(filter.bitmap) != id(filter1.bitmap) and id(filter.bitmap) != id(filter2.bitmap)
    assert id(filter1) != id(filter) and id(filter2) != id(filter)


def test_bloom_filter_in_real_scenario():
    func_dict = {
        Murmur32HashFunction: 5,
        MD5HashFunction: 5,
        SHA1HashFunction: 5
    }
